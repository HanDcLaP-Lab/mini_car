...
struct PIDController_DualPD ROT = {.Kp = 0.13, .Kp2 = 0.00008, .Kd = 0.03, .gKd = -0.082, .targetVal = 0, .currentError = 0, .preError = 0, .derivative = 0};
...
float computeMUXVal() {
    ...
    if (M.LEDCounter == 12) {
        ...
        SHARPlastside = 0;
        if (angle_effective(0, 30000) && (real_distance > 700000 || loop == 0)) {
            circletrigger[0] = 0;
            circletrigger[1] = 0;
            circletrigger[2] = 0;
            circletrigger[3] = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* htim) {
    if (htim == &htim2) {
        ...
        if(roundtime > 10600) defultSpeed = 86;
